...
import dev.pegasus.colorpickerview.flags.FlagView;
import dev.pegasus.colorpickerview.listeners.ColorEnvelopeListener;
import dev.pegasus.colorpickerview.listeners.ColorListener;
...
public class ColorPickerView extends FrameLayout implements LifecycleObserver {
    ...
    private final ColorPickerPreferenceManager preferenceManager = ColorPickerPreferenceManager.getInstance(getContext());
    ...
    private void getAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            if (a.hasValue(R.styleable.ColorPickerView_selector_alpha)) {
                this.selector_alpha = a.getFloat(R.styleable.ColorPickerView_selector_alpha, selector_alpha);
            }
            ...
            if (a.hasValue(R.styleable.ColorPickerView_flag_isFlipAble)) {
                this.flag_isFlipAble = a.getBoolean(R.styleable.ColorPickerView_flag_isFlipAble, flag_isFlipAble);
            }
            ...
            if (a.hasValue(R.styleable.ColorPickerView_debounceDuration)) {
                this.debounceDuration = a.getInteger(R.styleable.ColorPickerView_debounceDuration, (int) debounceDuration);
            }
    ...
    protected void onCreateByBuilder(Builder builder) {
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(SizeUtils.dp2Px(getContext(), builder.width), SizeUtils.dp2Px(getContext(), builder.height));
    ...
    protected int getColorFromBitmap(float x, float y) {
        ...
        if (palette.getDrawable() != null && palette.getDrawable() instanceof BitmapDrawable && mappedPoints[0] >= 0 && mappedPoints[1] >= 0 && mappedPoints[0] < palette.getDrawable().getIntrinsicWidth() && mappedPoints[1] < palette.getDrawable().getIntrinsicHeight()) {
    ...
    public void setInitialColor(@ColorInt final int color) {
        if (getPreferenceName() == null || (getPreferenceName() != null && preferenceManager.getColor(getPreferenceName(), -1) == -1)) {
            post(() -> {
                try {
