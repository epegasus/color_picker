...
import dev.pegasus.colorpickerview.ActionMode;
...
abstract class AbstractSlider extends FrameLayout {
    ...
    public void setSelectorByHalfSelectorPosition(@FloatRange(from = 0.0, to = 1.0) float selectorPosition) {
        this.selectorPosition = Math.min(selectorPosition, 1.0f);
        float x = (getMeasuredWidth() * selectorPosition) - (getSelectorSize() * 0.5f) - getBorderHalfSize();
        selectedX = (int) getBoundaryX(x);
        selector.setX(selectedX);
    }
